import { homeCardsVariants } from "@repo/shared/animations/card-transitions"
...
import type { AppLink, Tab } from "@/components/home/card-definitions"
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {appCards?.length > 0 ? (
          appCards.map(({ icon, title, links, description }) => {
              ...
              >
                <Link target="_blank" rel="noopener noreferrer" href={href}>
                  ...
                  <span className="relative">
                    {title}
                    {showWebhookStatusDot && !isLoadingWebhookUrl && !webhookUrl && (
      ...
      <FirstSignupDialog open={showDialog} onOpenChange={handleDialogOpenChange} chatId={chatId} />
    </motion.div>
  )
}
