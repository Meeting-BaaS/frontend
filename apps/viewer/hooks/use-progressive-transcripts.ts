...
export function useProgressiveTranscripts({
  ...
  const [meetingData, setMeetingData] = useState<MeetingDataResponse>(initialMeetingData)
  ...
  const hasTranscripts = (initialMeetingData.bot_data.transcripts?.length ?? 0) > 0
